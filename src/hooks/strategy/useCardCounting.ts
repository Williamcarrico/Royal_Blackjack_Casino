...
import { Card, Rank } from '../../types/cardTypes';
...
export function useCardCounting(
    ...
    const getStrategyDeviations = useCallback((
        ...
        if (canSurrender) {
            if (playerTotal === 15 && dealerUpCard === '10' && trueCount >= 4) {
                return 'Surrender';
            }
            if (playerTotal === 15 && dealerUpCard === '9' && trueCount >= 2) {
                return 'Surrender';
            }
        }
        ...
        if (canDouble) {
            ...
            if (playerTotal === 12 && ['2', '3'].includes(dealerUpCard) && trueCount >= 2) {
                return 'Stand';
            }
        }

        // Stand deviations
        if (playerTotal === 16 && dealerUpCard === '10' && trueCount >= 0) {
            return 'Stand';
        }
        if (playerTotal === 15 && dealerUpCard === '10' && trueCount >= 4) {
            return 'Stand';
