...
// Define the HandStore interface
interface HandStore {
    playerHands: Hand[];
    dealerHand: DealerHand | null;
    createHand: (playerId: string, betAmount: number) => Hand;
    addCardToDealerHand: (card: Card) => void;
    addCardToHand: (handId: string, card: Card) => void;
    evaluateHand: (handId: string) => void;
    getAvailableActions: (handId: string) => void;
    updateHand: (target: { type: string; id: string }, hand: Hand | DealerHand | Partial<Hand> | Partial<DealerHand>) => void;
    splitHand: (handId: string) => [Hand, Hand] | null;
    clearHands: () => void;
    removeHand: (handId: string) => void;
}

// Hand slice store
const useHandStore = create<HandStore>(createHandSlice);
...
export function useHand(gameOptions?: GameOptions) {
    ...
    const performSplit = useCallback((
        ...
        if (splitResult) {
            ...
            // Update the new hand's bet amount using newBetId
            const newBetAmount = parseInt(newBetId, 10) || hand.bet;
