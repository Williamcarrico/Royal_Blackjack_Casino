...
import { DEFAULT_CHIP_VALUES } from '@/lib/constants/chips';
import { useBetValidation } from '@/hooks/game/useBetValidation';
...
const BettingControls: React.FC<BettingControlsProps> = ({
    ...
    // Use our consolidated bet validation hook
    const { canPlaceBet, validateBet } = useBetValidation({
        minBet,
        maxBet,
        balance
    });
    ...
    const canSelectChip = useCallback((value: ChipValue): boolean => {
        ...
        return canPlaceBet(newBet);
    }, [isChipAvailable, disabledChips, currentBet, canPlaceBet]);
    ...
    const addChipToBet = useCallback((value: ChipValue): void => {
        const newBet = currentBet + value;
        const error = validateBet(newBet);
        if (error) {
    ...
    }, [currentBet, validateBet, onPlaceBet]);
    ...
    const handleChipSelect = useCallback((value: ChipValue): void => {
        ...
        if (canSelectChip(value)) {
            ...
            const newBet = currentBet + value;
            const error = validateBet(newBet);
            setErrorMessage(error ? error.message : `Cannot select $${value} chip`);
        }
    }, [disabled, canSelectChip, autoConfirm, onPlaceBet, validateBet, currentBet]);
    ...
    const handleConfirmBet = useCallback((): void => {
        const error = validateBet(currentBet);
        if (error) {
            setErrorMessage(error.message);
            return;
        }
    ...
    }, [currentBet, selectedChip, validateBet, addChipToBet]);
