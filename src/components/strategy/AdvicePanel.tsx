...
const AdvicePanel = ({
    ...
    // Displays user-friendly game phase
    const getGamePhaseDisplay = (phase: string): string => {
        switch (phase) {
            case 'betting': return 'Betting Phase';
            case 'dealing': return 'Dealing Cards';
            case 'player-turn': return 'Player Decision';
            case 'dealer-turn': return 'Dealer Turn';
            case 'payout': return 'Hand Complete';
            ...
            {!compact && (
                <CardFooter className="flex items-center justify-between px-4 py-3 text-xs border-t bg-black/5 dark:bg-white/5 text-muted-foreground">
                    ...
                    <div className="flex items-center">
                        ...
                        <span>{getGamePhaseDisplay(gamePhase)}</span>
                    </div>
                </CardFooter>
            )}
        </Card>
